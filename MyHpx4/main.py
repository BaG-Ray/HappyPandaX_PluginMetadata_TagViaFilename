...
def trans_Chinese(word):
    with open(r'/data/db.text.json', 'r', encoding='utf-8') as json_File:
        ...
        log.info("Now are using translate program")
...
def query(itemtuple, login_site=URLS['eh']):
    ...
    for mitem in itemtuple:
        ...
        file_Sources = item.get_sources()
        if file_Sources:
            # get folder/file name
            file_Name = os.path.split(file_Sources[0])[1]
            if "-" in file_Name:
                file_Name_List = file_Name.split("-")
                GID = file_Name_List[0]
                GTOKEN = file_Name_List[1]
                url = "https://exhentai.org/g/" + str(GID) + "/" + str(GTOKEN)
                log.info(f"Getting {url} Via Name for {item}")
            ...
            if url:
                ...
                if (ex_login if "exhentai" in login_site else True):
                    ...
                    if "-" in i_title:
                        file_List = i_title.split("-")
                        gID = file_List[0]
                        gToken = file_List[1]
                        gTitle = (file_List[2:]).join("-")
                        exUrl = "https://exhentai.org/g/" + str(gID) + "/" + str(gToken)
                        re_tuple = (gTitle, exUrl)
                        gurls.append(re_tuple)
...
def apply_metadata(data, gallery, options):
    ...
    if isinstance(data.get('artists'), (list, tuple, set)):
        ...
        for a, c in data['artists']:
            if a:
                gartist = ArtistData(names=[ArtistNameData(name=a)])
                gartists.append(gartist)

                if c:
                    gcircles = []
                    for circlename in [x for x in c if x]:
                        gcircles.append(CircleData(name=circlename))
    ...
    if isinstance(data.get('parodies'), (list, tuple, set)):
        ...
        for p in data['parodies']:
            if p:
                gparody = ParodyData(names=[ParodyNameData(name=trans_Chinese(p))])
    ...
    if isinstance(data.get('tags'), (dict, list)):
        ...
        for ns, tags in data['tags'].items():
            ...
            for t in tags:
                t = t.strip()
                if t:
                    kw = {'tag': TagData(name=trans_Chinese(t))}
                    if ns:
